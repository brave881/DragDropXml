...
import com.example.dragdropxml.config.ColumnProperties
import com.example.dragdropxml.config.DragItem
import com.example.dragdropxml.config.callbacks.BoardCallback
import com.example.dragdropxml.config.callbacks.BoardListener
import com.example.dragdropxml.config.utils.ColumnSnapPosition
import com.example.dragdropxml.databinding.BoardFragmentBinding


class BoardFragment : Fragment() {
    private lateinit var binding: BoardFragmentBinding

    private var createdItems = 0
    private var columns = 0
    private var gridLayout = false
    private var disableColumnDrag = true
    ...
    override fun onCreateView(
        ...
        binding =// inflater.inflate(R.layout.board_fragment, container, false)// as BoardFragment
            BoardFragmentBinding.inflate(layoutInflater)
        ...
        binding.boardView.setColumnSnapPosition(ColumnSnapPosition.CENTER)
        binding.boardView.setBoardListener(object : BoardListener {
        ...
        binding.boardView.setBoardCallback(object : BoardCallback {
            ...
            override fun canDragColumnAtPosition(index: Int): Boolean {
                // Add logic here to prevent a column to be dragged // TODO

                return disableColumnDrag
            }
...
//        }
    ...
    private fun resetBoard() {
        ...
        binding.boardView.setCustomDragItem(
            if (gridLayout) null else BoardFragment.MyDragItem(
                requireContext(), R.layout.column_item
            )
        )
        binding.boardView.setCustomColumnDragItem(
            if (gridLayout) null else MyColumnDragItem(
                requireContext(), R.layout.column_drag_layout
            )
        )
        addColumn()
        addColumn()
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        ...
        menu.findItem(R.id.action_grid).isVisible = !gridLayout
        menu.findItem(R.id.action_list).isVisible = gridLayout
    }
    ...
    private fun addColumn() {
        ...
        for (i in 0 until addItems) {
            val id = (createdItems++).toLong()
            mItemArray.add(Pair(id, "Item $id"))
        }
        val listAdapter = ItemAdapter(
            mItemArray,
            if (gridLayout) R.layout.grid_item else R.layout.column_item,
            R.id.item_layout,
            true
        )
        val header = View.inflate(activity, R.layout.column_header, null)
        (header.findViewById<View>(R.id.text) as TextView).text = "Column " + (columns + 1)
...
//        ((TextView) footer.findViewById(R.id.item_count)).setText("3");
        val layoutManager = if (gridLayout) GridLayoutManager(context, 4) else LinearLayoutManager(
            context
        )
        val columnProperties =
            ColumnProperties.Builder.newBuilder(listAdapter).setLayoutManager(layoutManager)
                ...
                .setItemsSectionBackgroundColor(Color.TRANSPARENT).setHeader(header)
                //  .setFooter(footer)
...
//            ((TextView) dragFooter.findViewById(R.id.item_count)).setText(((TextView) clickedFooter.findViewById(R.id.item_count)).getText());
        ...
        override fun onStartDragAnimation(dragView: View) {
            ...
            dragView.animate().scaleX(0.1f).scaleY(0.1f).start()
        }
    ...
    private class MyDragItem(context: Context, layoutId: Int) : DragItem(context, layoutId) {
        ...
        override fun onEndDragAnimation(dragView: View) {
            ...
            val anim = ObjectAnimator.ofFloat(
                dragCard, "CardElevation", dragCard.cardElevation, 6f
            )
            anim.interpolator = DecelerateInterpolator()
            anim.duration = ANIMATION_DURATION.toLong()
            anim.start()
